...
public class Main{
    ...
    public static void main(String[] args) {
        ...
        ArrayList <GRBVar> grbVars = new ArrayList <GRBVar>();
...
//        }
        ...
        //we need a method to solve the problem
        ...
        //GRB ENV model
        try {
        	GRBEnv env = new GRBEnv("Environment");
			GRBModel model = new GRBModel(env);
			//variables 
			for(int i = 0; i<cars.size();i++) {
				GRBVar tempVar = model.addVar(0, 1, 0, GRB.BINARY, "CAR "+(i+1));//x1 x2 x3 
				grbVars.add(tempVar);
			}
			...
			for(int i = 0; i<cars.size();i++) {
				con.addTerm(cars.get(i).getPrice(),grbVars.get(i));
			}
			model.addConstr(con, GRB.LESS_EQUAL, capacity,"Money constraint");
			...
			for (int i = 0; i < cars.size(); i++) {
				expr.addTerm(cars.get(i).value(), grbVars.get(i));
			}
    ...
    public static int [][] readFromCSV(String fileName){
        ...
        File file = new File("/Users/kaganozturkoglu/eclipse-workspace/GUROBI27/src/cars.csv");
